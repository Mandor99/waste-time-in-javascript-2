...
const generatedSerial = document.querySelector('.generated');

// form
// 1 - clear placeholder
function clearPlaceHolder() {
    const placeHolder = inputName.getAttribute('placeholder');
    inputName.onblur = (e) => { e.target.placeholder = placeHolder; };
    inputName.onfocus = (e) => { e.target.placeholder = ''; };
}
...
function copyContent(copiedElement, contentElement) {
    copiedElement.onkeyup = () => { contentElement.textContent = copiedElement.value; };
}
/** ************************************************************************** */
// 5 - count down
let time = 115;
...
function redirectWebsite(url) {
    redirect.addEventListener('click', () => { url !== '' ? window.location = url : false; });
}
...
colorsArr.forEach((ele) => {
    ...
    ele.addEventListener('click', () => {
        ...
        section.classList.add(ele.getAttribute('data-color'));
